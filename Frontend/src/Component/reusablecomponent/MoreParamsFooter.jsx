...
const MoreParamsFooter = ({ sectionKey, darkMode }) => {
    ...
    return (
        <div className={`w-full ${darkMode ? "bg-gray-900" : "bg-gray-50"} pb-12`}>
            <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div
                    className={`flex flex-col sm:flex-row items-center justify-between gap-4 rounded-2xl border border-dashed px-6 py-5 ${darkMode ? "bg-slate-900/40 border-slate-700 text-slate-400" : "bg-white border-slate-300 text-slate-500"
                        }`}
                >
                    <div className="flex items-center gap-3 text-sm text-center sm:text-left">
                        <Code2 className={`w-5 h-5 shrink-0 ${darkMode ? "text-blue-400" : "text-blue-500"}`} />
                        <span>
                            You're viewing dealer parameters.{" "}
                            <span className={darkMode ? "text-slate-200 font-semibold" : "text-slate-700 font-semibold"}>
                    ...
                    <button
                        ...
                        className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white bg-blue-600 hover:bg-blue-500 shadow-md shadow-blue-600/20 transition-all active:scale-[0.98] shrink-0"
                    >
