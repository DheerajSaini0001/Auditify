  ...
  return (
    <div className="relative flex-shrink-0">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-0.5">
        <span className={`text-3xl font-black tracking-tight ${darkMode ? "text-white" : "text-ink"}`}>
          {score}%
        </span>
...
const FakeStat = ({ icon: Icon, label, value, color, darkMode }) => (
  <div className={`flex items-center gap-2 text-sm ${darkMode ? "text-slate-400" : "text-muted"}`}>
...
const FakeCard = ({ darkMode, w = "col-span-1" }) => (
  <div className={`${w} h-28 rounded-2xl border blur-[2px] opacity-40 ${darkMode ? "bg-slate-800 border-slate-700" : "bg-card border-line"}`} />
);
...
const GuestReportPage = () => {
  ...
  return (
    <div className={`w-full min-h-screen ${darkMode ? "bg-gray-900" : "bg-surface"}`}>

      {/* ── URL HEADER CARD ── */}
      <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-0 pb-8`}>
        <div className={`rounded-3xl overflow-hidden transition-all duration-300 ${darkMode
          ...
          <div className={`relative p-6 md:p-8 border-b ${darkMode ? "border-slate-800 bg-slate-900/50" : "border-line bg-cardsoft"}`}>
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                  <span className={`text-2xl md:text-3xl font-semibold ${darkMode ? "text-white/40" : "text-faint"} italic`}>
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold border ${darkMode ? "bg-slate-800/50 border-slate-700/50 text-slate-400" : "bg-cardsoft border-line text-muted"}`}>
          ...
          <div className={`flex flex-col xl:flex-row min-h-[260px]`}>
            ...
            <div className={`w-full xl:w-[45%] p-6 flex items-center justify-center border-b xl:border-b-0 xl:border-r relative overflow-hidden ${darkMode ? "bg-slate-900/40 border-slate-800" : "bg-cardsoft border-line"}`}>
              <div className="absolute inset-0 bg-blue-500/5 blur-3xl rounded-full" />
              <div className={`relative z-10 w-full h-40 rounded-2xl border flex items-center justify-center ${darkMode ? "bg-slate-800/60 border-slate-700" : "bg-cardsoft border-line"}`}>
                <div className="text-center space-y-2 opacity-30">
                  <Globe size={32} className={darkMode ? "text-slate-400 mx-auto" : "text-faint mx-auto"} />
            ...
            <div className="flex-1 px-6 pb-6 pt-6 lg:px-12 lg:pt-8 flex flex-col justify-center">
              <div className="max-w-2xl mx-auto w-full space-y-6">
                <div className="flex flex-col md:flex-row items-center gap-8 md:gap-8 justify-center">

                  {/* Text */}
                  <div className="flex-1 space-y-4 text-left order-2 md:order-1">
                    <div>
                      <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-semibold uppercase tracking-wider mb-3 ${darkMode ? "bg-blue-500/10 text-blue-400 border border-blue-500/20" : "bg-accentsoft text-accent border border-accent/20"
                        }`}>
                        <BarChart2 className="w-3.5 h-3.5" />
                        {meta.badge}
                      </div>
                      <h2 className={`text-2xl lg:text-4xl font-black tracking-tight ${darkMode ? "text-white" : "text-ink"}`}>
                        {meta.label.split(" ")[0]}{" "}
                        <span className={meta.color}>{meta.label.split(" ").slice(1).join(" ")}</span>
                      </h2>
                      <p className={`text-sm leading-relaxed opacity-60 mt-2 ${darkMode ? "text-slate-300" : "text-muted"}`}>
        ...
        <div className="relative mt-6">
          ...
          {/* Lock Overlay */}
          <div className={`absolute inset-0 z-10 flex items-center justify-center p-4 rounded-3xl backdrop-blur-sm ${darkMode ? "bg-slate-950/60" : "bg-surface/70"
            }`}>
            <div className={`max-w-xl w-full p-10 rounded-[2.5rem] text-center shadow-2xl border transition-all duration-300 ${darkMode
              ? "bg-slate-900 border-slate-800 shadow-black/40"
              : "bg-card border-line shadow-slate-200/50"
              }`}>

              {/* Lock Icon */}
              <div className="mb-6 relative flex justify-center">
                <div className="absolute inset-0 bg-blue-500/20 rounded-full blur-2xl animate-pulse" />
                <div className={`relative p-6 rounded-full border shadow-inner ${darkMode ? "bg-slate-800 border-slate-700" : "bg-cardsoft border-line"
                  }`}>
                  <Lock className={`w-10 h-10 ${darkMode ? "text-blue-400" : "text-accent"}`} />
                </div>
              </div>

              <h2 className={`text-2xl md:text-3xl font-black tracking-tight mb-3 ${darkMode ? "text-white" : "text-ink"}`}>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                ...
                <span className={`text-sm font-semibold uppercase tracking-widest opacity-40 ${darkMode ? "text-slate-500" : "text-faint"}`}>
                  or
                </span>

                <button
                  onClick={handleRegister}
                  className="flex items-center justify-center gap-2 px-7 py-3.5 rounded-2xl font-semibold transition-all duration-300 w-full sm:w-auto shadow-lg hover:shadow-xl active:scale-95 bg-accent hover:bg-accenthover text-white"
                >
                  <UserPlus size={17} />
