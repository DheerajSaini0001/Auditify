  ...
  return (
    <div className={containerClass}>
      ...
      <div className="relative z-10 max-w-5xl w-full mx-auto space-y-8">
        ...
        {/* Header */}
        <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8 mb-10 border-b pb-8 border-slate-500/20">
          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 px-6 py-2 rounded-full bg-accentsoft text-accent text-sm font-black tracking-widest uppercase border border-accent/20">
              <Code size={16} /> API Reference
            </div>
            <h1 className="text-5xl md:text-6xl font-black tracking-tighter">Automate Everything.</h1>
            <p className={`text-lg max-w-md ${darkMode ? "text-slate-400" : "text-muted"}`}>
              Integrate world-class auditing directly into your CI/CD pipelines, dashboards, and automated workflows.
            </p>
          </div>
          <div className={`p-6 rounded-2xl border backdrop-blur-md ${darkMode ? "bg-blue-500/5 border-blue-500/20" : "bg-card border-line shadow-xl"}`}>
            <h4 className="font-semibold text-sm mb-2 opacity-70 uppercase tracking-widest">Base URL</h4>
            <div className={`text-lg font-mono font-semibold ${darkMode ? "text-blue-500" : "text-accent"}`}>https://api.Site Audit.com/v1</div>
        ...
        {/* Auth Section */}
        <div className={`relative overflow-hidden rounded-3xl border transition-all duration-300 ${darkMode ? "bg-[#09101C] border-slate-800" : "bg-card border-line shadow-2xl shadow-slate-200/50"}`}>
          <div className="absolute top-0 w-full h-1 bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500"></div>
          <div className="p-8 md:p-12 md:flex gap-8 items-center">
            <div className="flex-1 space-y-6">
              <div className="w-12 h-12 rounded-xl bg-accentsoft flex items-center justify-center text-accent">
                <Key size={24} />
              </div>
              <h2 className="text-3xl font-semibold tracking-tight">Authentication</h2>
              <p className={`text-lg leading-relaxed ${darkMode ? "text-slate-400" : "text-muted"}`}>
                Requests to the Site Audit API must be authenticated using a Bearer token. Create API Keys directly inside your developer dashboard. We recommend cycling these keys every 90 days.
              </p>
            </div>
        ...
        <div className={`relative overflow-hidden rounded-3xl border transition-all duration-300 ${darkMode ? "bg-[#09101C] border-slate-800" : "bg-card border-line shadow-2xl shadow-slate-200/50"}`}>
          ...
          <div className="p-8 md:p-12 md:flex flex-row-reverse gap-8 items-center">
            <div className="flex-1 space-y-6">
              ...
              <p className={`text-lg leading-relaxed ${darkMode ? "text-slate-400" : "text-muted"}`}>
                Kick off a headless scanning sequence by sending a POST request containing the target URL. Because auditing takes time, this endpoint returns immediately with a <code className={`px-2 py-1 rounded-md text-sm ${darkMode ? "bg-slate-800" : "bg-cardsoft"}`}>job_id</code>. Process completion is transmitted automatically to your registered Webhook URI.
              </p>
            </div>
            <div className={`flex-1 rounded-2xl border overflow-hidden mt-8 md:mt-0 ${darkMode ? "bg-[#0B1221] border-slate-800" : "bg-slate-900 border-slate-800"}`}>
              <div className="flex px-4 py-3 border-b border-white/10 items-center justify-between">
              ...
              <div className="p-6 font-mono text-sm leading-8 text-slate-300 overflow-x-auto">
                <span className="text-blue-400">curl</span> -X POST https://api.Site Audit.com/v1/scan \<br />
