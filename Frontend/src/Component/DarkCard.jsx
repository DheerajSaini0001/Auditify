...
export default function DarkCard() {
  ...
  const [shouldNavigate, setShouldNavigate] = useState(false); // ✅ New state added
  ...
  const handleClick = async (e) => {
    ...
    setShouldNavigate(true); // ✅ Allow navigation after fetch completes
  };

  // ✅ Navigate to /report only AFTER user triggers it manually
  useEffect(() => {
    if (firstLoad) {
      setFirstLoad(false);
      return;
    ...
    if (shouldNavigate && !loading && data && location.pathname !== "/report") {
      navigate("/report", { state: { from: "/", data }, replace: true });
      setInputValue("");
      setShouldNavigate(false); // reset flag
    }
  }, [data, loading, navigate, location.pathname, firstLoad, shouldNavigate]);
    ...
    >
      <div className="mx-6 mt-32 sm:mx-0 w-full backdrop-blur-xl">
        ...
        >
          ...
          >
            Enter a URL, select a device and report type, then click Analyze.
          </p>
