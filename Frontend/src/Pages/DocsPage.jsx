...
export default function DocsPage() {
  ...
  return (
    <div className={containerClass}>
      ...
      <div className={`fixed top-[-20%] left-[-10%] w-[50%] h-[50%] rounded-full mix-blend-screen filter blur-[120px] opacity-30 ${darkMode ? 'bg-violet-600' : 'bg-violet-300'} pointer-events-none`} />
      <div className={`fixed bottom-[-20%] right-[-10%] w-[50%] h-[50%] rounded-full mix-blend-screen filter blur-[120px] opacity-20 ${darkMode ? 'bg-purple-600' : 'bg-purple-300'} pointer-events-none`} />

      <div className="relative z-10 max-w-5xl w-full mx-auto space-y-16">
        
        {/* Hero Section */}
        <div className="text-center space-y-6">
          <div className="inline-flex items-center gap-2 px-6 py-2 rounded-full bg-violet-500/10 text-violet-500 text-sm font-black tracking-widest uppercase border border-violet-500/20 shadow-[0_0_20px_rgba(139,92,246,0.2)]">
            <BookOpen size={16} /> Auditify Documentation
          </div>
          <h1 className="text-5xl md:text-7xl font-black tracking-tighter leading-tight">
            Understand the <br/>
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-violet-400 via-purple-400 to-indigo-500 drop-shadow-sm">
              Intelligence Engine.
            </span>
          </h1>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          {/* Card 1 */}
          <div className={`group relative p-8 rounded-3xl transition-all duration-500 border overflow-hidden ${darkMode ? 'bg-slate-900/40 border-slate-700/50 hover:bg-slate-800/80 hover:border-violet-500/50 hover:shadow-[0_0_40px_rgba(139,92,246,0.1)]' : 'bg-white border-slate-200 shadow-xl shadow-slate-200/40 hover:border-violet-400/50 hover:shadow-[0_10px_40px_rgba(139,92,246,0.15)]'}`}>
            <div className={`absolute top-0 right-0 p-32 bg-violet-500/10 rounded-full blur-[80px] -mr-16 -mt-16 transition-opacity group-hover:opacity-100 ${darkMode ? 'opacity-0' : 'opacity-50'}`}></div>
            <div className="relative z-10">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-violet-400 to-purple-500 flex items-center justify-center text-white mb-6 shadow-lg shadow-violet-500/30">
                <Globe size={28} />
              </div>
              ...
              <div className={`font-mono text-xs p-4 rounded-xl ${darkMode ? 'bg-slate-950 border border-slate-800 text-violet-400' : 'bg-slate-50 border border-slate-200 text-violet-600'}`}>
          ...
          {/* Card 2 */}
          <div className={`group relative p-8 rounded-3xl transition-all duration-500 border overflow-hidden ${darkMode ? 'bg-slate-900/40 border-slate-700/50 hover:bg-slate-800/80 hover:border-violet-500/50 hover:shadow-[0_0_40px_rgba(59,130,246,0.1)]' : 'bg-white border-slate-200 shadow-xl shadow-slate-200/40 hover:border-violet-400/50 hover:shadow-[0_10px_40px_rgba(59,130,246,0.15)]'}`}>
            <div className={`absolute top-0 right-0 p-32 bg-violet-500/10 rounded-full blur-[80px] -mr-16 -mt-16 transition-opacity group-hover:opacity-100 ${darkMode ? 'opacity-0' : 'opacity-50'}`}></div>
            <div className="relative z-10">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-violet-400 to-indigo-500 flex items-center justify-center text-white mb-6 shadow-lg shadow-violet-500/30">
                <Activity size={28} />
              </div>
              ...
              <ul className={`mt-6 space-y-3 font-semibold text-sm ${darkMode ? "text-slate-400" : "text-slate-500"}`}>
                <li className="flex items-center gap-3"><ChevronRight size={16} className="text-violet-500"/> Largest Contentful Paint (25%)</li>
                <li className="flex items-center gap-3"><ChevronRight size={16} className="text-violet-500"/> Total Blocking Time (25%)</li>
                <li className="flex items-center gap-3"><ChevronRight size={16} className="text-indigo-500"/> Cumulative Layout Shift (15%)</li>
              </ul>
            </div>
          </div>
