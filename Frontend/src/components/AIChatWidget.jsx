...
const AIChatWidget = () => {
    ...
    return (
        <div className={`fixed bottom-6 right-6 w-[350px] sm:w-[400px] h-[500px] max-h-[80vh] flex flex-col rounded-2xl shadow-2xl z-50 transition-all duration-300 transform origin-bottom-right border ${darkMode ? 'bg-[#1E2833] border-slate-700 shadow-blue-500/10' : 'bg-card border-line'}`}>
            ...
            {/* Header */}
            <div className={`p-4 rounded-t-2xl flex items-center justify-between border-b ${darkMode ? 'bg-gradient-to-r from-[#303945] to-[#1E2833] border-slate-700 text-white' : 'bg-surface text-ink border-line'}`}>
            ...
            {/* Chat Body */}
            <div className={`flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar ${darkMode ? 'bg-[#1E2833]' : 'bg-surface-2'}`}>
            ...
            {/* Input Area */}
            <form onSubmit={handleSend} className={`p-4 border-t ${darkMode ? 'bg-[#303945] border-slate-700' : 'bg-card border-line'}`}>
