    ...
    return (
        <div className={`flex items-center ${className}`}>
            <div className="flex items-center gap-2">
                ...
                <span className={`text-xs   tracking-widest ${darkMode ? "text-slate-200" : "text-slate-500"}`}>{passedCount} Passed</span>
            </div>
            <div className="flex items-center gap-2">
                <XCircle size={18} className="text-rose-500" />
                <span className={`text-xs   tracking-widest ${darkMode ? "text-slate-200" : "text-slate-500"}`}>{failedCount} Failed</span>
            </div>
            <div className="flex items-center gap-2">
                <AlertTriangle size={18} className="text-amber-500" />
                <span className={`text-xs   tracking-widest ${darkMode ? "text-slate-200" : "text-slate-500"}`}>{warningCount} Warning</span>
            </div>
        </div>
    );
};
