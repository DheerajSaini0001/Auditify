...
const AdminDashboard = () => {
  ...
  return (
    <div className={`min-h-screen pt-20 transition-colors duration-300 ${darkMode ? 'bg-[#0a0a0f] text-white' : 'bg-slate-50 text-slate-900'}`}>
      <div className="max-w-[1600px] mx-auto flex gap-6 p-4 sm:p-6 lg:p-8">
        
        {/* Professional Sidebar */}
        <aside className="hidden lg:flex flex-col w-64 gap-8 shrink-0">
          <div className="space-y-6">
            <div className="px-4">
              <h2 className={`text-[10px] font-black uppercase tracking-[0.2em] ${darkMode ? 'text-gray-500' : 'text-slate-400'}`}>Main Menu</h2>
              <div className="mt-4 space-y-1">
                <SidebarItem icon={LayoutDashboard} label="Overview" id="overview" active={activeTab === 'overview'} />
                <SidebarItem icon={Users} label="User Management" id="users" active={activeTab === 'users'} />
                <SidebarItem icon={Globe} label="Audit Logs" id="logs" active={activeTab === 'logs'} />
                <SidebarItem icon={Settings} label="System Config" id="settings" active={activeTab === 'settings'} />
              </div>
            </div>

            <div className="px-4">
              <h2 className={`text-[10px] font-black uppercase tracking-[0.2em] ${darkMode ? 'text-gray-500' : 'text-slate-400'}`}>Quick Actions</h2>
              <div className="mt-4 space-y-2">
                <button className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold border border-dashed ${darkMode ? 'border-white/10 text-gray-400 hover:text-white hover:border-white/20' : 'border-slate-200 text-slate-500 hover:text-slate-900 hover:border-slate-300'}`}>
                  <UserPlus size={16} />
          ...
          <div className="mt-auto px-4 pb-4">
            <div className={`p-4 rounded-2xl border ${darkMode ? 'bg-white/5 border-white/5' : 'bg-white border-slate-200'}`}>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-blue-600 flex items-center justify-center font-bold text-white">A</div>
        ...
        <main className="flex-1 space-y-8">
          ...
          <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-blue-500 mb-1">
                <span>Admin</span>
                <ChevronRight size={10} />
                <span className={darkMode ? 'text-gray-400' : 'text-slate-400'}>
                  {activeTab === 'overview' ? 'Overview' : activeTab === 'users' ? 'Users' : activeTab === 'logs' ? 'Audit Logs' : 'Settings'}
                </span>
              </div>
              <h1 className="text-3xl font-black tracking-tight">
                {activeTab === 'overview' ? 'Platform Dashboard' : activeTab === 'users' ? 'Manage Users' : activeTab === 'logs' ? 'Website Audits' : 'System Settings'}
              </h1>
            </div>

            <div className="flex items-center gap-3">
              <button className={`p-3 rounded-xl border transition-all ${darkMode ? 'bg-white/5 border-white/5 hover:bg-white/10' : 'bg-white border-slate-200 hover:bg-slate-50'}`}>
                <Bell size={20} className="text-gray-500" />
              </button>
              <button className={`flex items-center gap-2 px-5 py-3 bg-blue-600 text-white rounded-xl font-bold shadow-lg shadow-blue-500/20 hover:scale-[1.02] transition-transform active:scale-[0.98]`}>
                <RefreshCw size={18} />
            ...
            {activeTab === 'overview' && (
              ...
              >
                ...
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                  {/* Main Activity Chart / Recent Area */}
                  <div className={`xl:col-span-8 border rounded-[2.5rem] p-8 transition-colors relative overflow-hidden ${darkMode ? 'bg-[#16161e]/50 border-white/5' : 'bg-white border-slate-200'}`}>
                    <div className="flex items-center justify-between mb-10">
                      <div>
                        <h3 className="text-xl font-black">Platform Activity</h3>
                        <p className="text-xs text-gray-500 mt-1">Daily scan volume over the last 30 days.</p>
                      </div>
                      <div className="flex items-center gap-2">
                        {['7D', '30D', '90D'].map(t => (
                          <button key={t} className={`text-[10px] font-black px-4 py-2 rounded-xl border transition-all ${t === '30D' ? 'bg-blue-600 text-white border-blue-500' : `${darkMode ? 'bg-white/5 border-white/5 text-gray-500' : 'bg-slate-50 border-slate-200 text-slate-500'}`}`}>
                            {t}
                          </button>
                        ))}
                      </div>
                    ...
                    {/* Placeholder for Recharts or Custom Chart */}
                    <div className="h-[320px] flex items-end gap-1.5 px-2">
                       {[40, 60, 45, 90, 65, 80, 55, 70, 85, 40, 60, 75, 90, 50, 65, 80, 45, 70, 95, 60, 75, 40, 55, 85, 70, 90, 65, 50, 80, 100].map((h, i) => (
                         <motion.div 
                           key={i}
                           initial={{ height: 0, opacity: 0 }}
                           animate={{ height: `${h}%`, opacity: 1 }}
                           transition={{ delay: i * 0.015, duration: 0.6, ease: "easeOut" }}
                           className={`flex-1 rounded-t-lg group relative transition-all duration-300 hover:scale-x-125 hover:brightness-125 ${darkMode ? 'bg-gradient-to-t from-blue-600/10 via-blue-500/40 to-blue-400/80' : 'bg-gradient-to-t from-blue-100 via-blue-300 to-blue-500'}`}
                         >
                            <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-black text-white text-[8px] font-black px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
                              {Math.floor(h * 12.5)} Scans
                            </div>
                         </motion.div>
                       ))}
                    </div>
                    <div className="flex justify-between mt-6 text-[10px] font-bold text-gray-500 uppercase tracking-widest px-4 border-t border-white/5 pt-4">
                  ...
                  <div className="xl:col-span-4 space-y-6">
                    <div className={`border rounded-[2.5rem] p-8 transition-colors ${darkMode ? 'bg-[#16161e]/50 border-white/5' : 'bg-white border-slate-200'}`}>
                      ...
                      <div className="space-y-6">
                        {[
                          { label: "API Latency", value: "42ms", trend: -12, type: "success" },
                          { label: "Worker Queue", value: "8 Pending", trend: 5, type: "info" },
                          { label: "Storage Capacity", value: "34%", trend: 2, type: "success" },
                          { label: "Active Sessions", value: "1.2k", trend: 18, type: "info" }
                        ].map((stat, i) => (
                          <div key={i} className="flex items-center justify-between">
                            ...
                            <div className={`text-[10px] font-black px-2 py-1 rounded-lg ${stat.trend < 0 ? 'bg-emerald-500/10 text-emerald-500' : 'bg-blue-500/10 text-blue-500'}`}>
                    ...
                    <div className={`border rounded-[2.5rem] p-8 transition-colors overflow-hidden relative group ${darkMode ? 'bg-indigo-600/10 border-indigo-500/20' : 'bg-indigo-50 border-indigo-100'}`}>
                       <h3 className="font-black text-indigo-500 text-xs uppercase tracking-widest mb-1">Audit Efficiency</h3>
                       <p className="text-3xl font-black mb-6">99.8%</p>
                       <div className="w-full h-3 rounded-full bg-indigo-500/10 overflow-hidden mb-2">
                         <motion.div 
                           initial={{ width: 0 }}
                           animate={{ width: "99.8%" }}
                       ...
                       <p className="text-[10px] font-bold text-indigo-600/60 uppercase tracking-tighter">Optimal performance reached</p>
                       <ShieldCheck className="absolute -right-6 -bottom-6 text-indigo-500/10 transition-transform group-hover:scale-110" size={140} />
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                   <div className={`border rounded-[2rem] p-6 ${darkMode ? 'bg-[#16161e]/50 border-white/5' : 'bg-white border-slate-200'}`}>
                      <h4 className="font-black text-sm uppercase tracking-widest text-blue-500 mb-4">Top Regions</h4>
                      <div className="space-y-4">
                        {[
                          { country: "United States", count: "1,240", percent: 45 },
                          { country: "United Kingdom", count: "580", percent: 22 },
                          { country: "India", count: "420", percent: 16 },
                          { country: "Germany", count: "210", percent: 8 }
                        ].map((r, i) => (
                          <div key={i} className="space-y-1.5">
                            ...
                            <div className="w-full h-1.5 rounded-full bg-gray-500/10 overflow-hidden">
                   ...
                   <div className={`border rounded-[2rem] p-6 ${darkMode ? 'bg-[#16161e]/50 border-white/5' : 'bg-white border-slate-200'}`}>
                      <h4 className="font-black text-sm uppercase tracking-widest text-emerald-500 mb-4">Device Usage</h4>
                      <div className="flex items-center justify-center h-[160px] relative">
                         {/* Simple visual ring chart placeholder */}
                         <div className="w-32 h-32 rounded-full border-[12px] border-emerald-500/10 flex items-center justify-center">
                            <div className="text-center">
                               <p className="text-2xl font-black">72%</p>
                               <p className="text-[8px] font-bold opacity-50 uppercase">Desktop</p>
                            </div>
                         </div>
                         <div className="absolute top-1/2 left-1/2 w-40 h-40 rounded-full border-[12px] border-transparent border-t-emerald-500 -translate-x-1/2 -translate-y-1/2 rotate-[120deg]" />
                      </div>
                      <div className="flex justify-center gap-6 mt-4">
                         <div className="flex items-center gap-2">
                           <div className="w-2 h-2 rounded-full bg-emerald-500" />
                           <span className="text-[10px] font-bold opacity-60">Desktop</span>
                         </div>
                         <div className="flex items-center gap-2">
                           <div className="w-2 h-2 rounded-full bg-emerald-500/20" />
                           <span className="text-[10px] font-bold opacity-60">Mobile</span>
                         </div>
                      </div>
                   </div>

                   <div className={`border rounded-[2rem] p-6 flex flex-col justify-between ${darkMode ? 'bg-blue-600 border-blue-500' : 'bg-blue-600 border-blue-700'} text-white relative overflow-hidden`}>
                      <div className="relative z-10">
                        <h4 className="font-black text-xs uppercase tracking-[0.2em] opacity-60 mb-2">Announcement</h4>
                        <p className="text-xl font-black leading-tight">Version 2.0.4 is now rolling out to all users.</p>
                      </div>
                      <button className="relative z-10 w-fit px-6 py-2 bg-white text-blue-600 rounded-xl font-black text-xs mt-6 hover:scale-105 transition-transform">
                        Read Changelog
                      </button>
                      <Zap className="absolute -right-8 -bottom-8 opacity-10" size={160} />
                   </div>
                </div>
              </motion.div>
            )}
                  ...
                  <div className={`px-8 py-6 border-t flex items-center justify-between ${darkMode ? 'border-white/5' : 'border-slate-100'}`}>
                    ...
                    <div className="flex gap-2">
                      <button 
                        ...
                        disabled={page === 1}
                        className="px-4 py-2 rounded-xl border text-xs font-bold disabled:opacity-30 transition-colors hover:bg-blue-600 hover:text-white"
                      >
                        Previous
                      </button>
                      <button 
            ...
            {activeTab === 'settings' && (
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {/* General Config */}
                  <div className={`border rounded-[2rem] p-8 transition-colors ${darkMode ? 'bg-[#16161e]/50 border-white/5' : 'bg-white border-slate-200'}`}>
                    ...
                    <div className="space-y-6">
                      {[
                        ...
                        <div key={i} className="flex items-center justify-between py-4 border-b border-white/5 last:border-0">
                          ...
                          <div className={`w-11 h-6 rounded-full relative cursor-pointer transition-colors ${item.toggle ? 'bg-blue-600' : 'bg-gray-700'}`}>
                    ...
                    <div className="space-y-6">
                      {[
                        ...
                        <div key={i} className="flex items-center justify-between py-4 border-b border-white/5 last:border-0">
                          ...
                          <div className={`w-11 h-6 rounded-full relative cursor-pointer transition-colors ${item.toggle ? 'bg-emerald-600' : 'bg-gray-700'}`}>
                            <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all shadow-sm ${item.toggle ? 'left-6' : 'left-1'}`} />
                          </div>
                        </div>
                      ))}
