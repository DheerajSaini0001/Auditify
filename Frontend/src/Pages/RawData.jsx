...
const RawData = ({ data, darkMode }) => {
  ...
  const innerCardClass = darkMode
    ...
    : "bg-cardsoft border border-linesoft";

  const textPrimary = darkMode ? "text-white" : "text-ink";
  const textSecondary = darkMode ? "text-slate-400" : "text-muted";
        ...
        <div className={`rounded-3xl border overflow-hidden shadow-xl ${masterCardClass}`}>
          <div className="p-8 md:p-12">
            ...
            <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
              <div>
                <h2 className={`text-3xl font-semibold tracking-tight mb-3 ${textPrimary}`}>Export & Data Payload</h2>
              ...
              <div className={`p-8 rounded-2xl flex flex-col transition-all duration-300 hover:-translate-y-1 ${innerCardClass}`}>
                <div>
                  <div className="flex items-center gap-4 mb-6">
                    <div className={`p-3.5 rounded-xl shadow-inner ${darkMode ? "bg-blue-500/10 text-blue-400" : "bg-accentsoft text-accent"}`}>
                      <FileText className="w-8 h-8" strokeWidth={1.5} />
                    </div>
                    <div>
                      <h3 className={`text-xl font-semibold ${textPrimary}`}>PDF Report</h3>
                      <span className={`text-xs font-semibold uppercase tracking-wider ${darkMode ? "text-blue-400" : "text-accent"}`}>Document</span>
                    </div>
                  </div>
                ...
                <div className="mt-auto pt-8">
                  <button
                    ...
                    className="group w-full flex items-center justify-center gap-3 py-4 rounded-xl text-sm font-semibold text-white bg-accent hover:bg-accenthover shadow-lg shadow-blue-600/20 transition-all hover:shadow-blue-600/30 active:scale-[0.98]"
                  >
              ...
              <div className={`p-8 rounded-2xl flex flex-col transition-all duration-300 hover:-translate-y-1 ${innerCardClass}`}>
                <div>
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      <h3 className={`text-xl font-semibold ${textPrimary}`}>Raw JSON Data</h3>
                ...
                <div className="mt-auto pt-8">
                  <button
                    ...
                    className={`w-full flex items-center justify-center gap-2 py-4 rounded-xl text-sm font-semibold border transition-all active:scale-[0.98] ${darkMode
                      ? "border-slate-600 text-slate-300 hover:bg-slate-700 hover:text-white"
                      : "border-line text-inksoft hover:bg-card hover:border-line hover:text-ink shadow-sm"
                      }`}
                  >
                ...
                >
                  <div className={`rounded-2xl border overflow-hidden shadow-2xl ${darkMode ? "bg-[#0f172a] border-slate-800" : "bg-card border-line"}`}>
                    ...
                    <div className={`flex items-center justify-between px-6 py-4 border-b ${darkMode ? "bg-slate-900 border-slate-800" : "bg-surface-2 border-line"}`}>
                      ...
                      <button
                        onClick={handleCopy}
                        className={`flex items-center gap-2 text-xs font-semibold px-4 py-2 rounded-lg transition-all ${copied
                          ? "text-emerald-600 bg-emerald-50 border border-emerald-100"
                          : darkMode ? "text-slate-400 hover:text-white hover:bg-slate-800 border border-transparent hover:border-slate-700" : "text-muted hover:text-ink hover:bg-card border border-transparent hover:border-line hover:shadow-sm"
                          }`}
                      >
                    ...
                    {/* Code Area */}
                    <div className={`p-6 overflow-auto max-h-[600px] custom-scrollbar text-sm font-mono leading-relaxed ${darkMode ? "text-blue-300 bg-[#0B1120]" : "text-indigo-700 bg-surface-2"}`}>
                      <pre>{jsonString}</pre>
                    </div>
                  </div>
                </motion.div>
